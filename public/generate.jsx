// Admin · Generate questions with AI.
// Pick subject + topics + difficulty mix + count (1-10), then preview/approve.
...
function AdminGenerate({ go }){
  const [subject, setSubject]   = useStateG('R&W');
  const [selected, setSelected] = useStateG(['trans','data-inf']);
  const [count, setCount]       = useStateG(5);
  const [mix, setMix]           = useStateG({ Easy: 20, Medium: 60, Hard: 20 });
  const [style, setStyle]       = useStateG('Match the official Bluebook tone. Vietnamese cultural context welcome but not required.');
  const [generated, setGenerated] = useStateG(null);     // null | array
  ...
  // when subject changes, clear selections that don't apply
  ...
  // Per-topic count breakdown — distribute as evenly as possible
  const breakdown = useMemoG(() => {
  ...
  }, [selected, count, subject]);

  // Difficulty pill counts (rough split based on sliders)
  const diffCounts = useMemoG(() => {
    ...
    <div className="view screen" data-screen-label="Admin · Generate questions">
      <div className="row between" style={{marginBottom: 22}}>
        <div>
          ...
          <p className="muted" style={{marginTop: 6, fontSize: 14, maxWidth: 620}}>
            Draft 1–10 SAT-style questions per run, scoped to the topics you select. Generated drafts land in the AI review queue for approval before going live.
          </p>
        </div>
        <div className="row" style={{gap:10}}>
          <span className="tag"><ISpark size={11}/> Powered by Claude Sonnet</span>
          <a className="btn btn-quiet" href="#" onClick={e=>e.preventDefault()}>View prompt template</a>
        </div>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1.4fr 1fr', gap:24, alignItems:'flex-start'}}>
        ...
        <div className="col" style={{gap: 24}}>
          ...
          <SectionG title="2 · Topics" hint={`${selected.length} selected`}
                    action={
                      <div className="row" style={{gap:8}}>
                        <button className="btn btn-ghost" style={{padding:'6px 10px', fontSize:12}} onClick={() => setSelected(allInSubject)}>
                          <ICheck/> Select all
                        </button>
                        <button className="btn btn-ghost" style={{padding:'6px 10px', fontSize:12}} onClick={() => setSelected([])}>
                          <IX/> Clear
                        </button>
                      </div>}>
            <p className="small muted" style={{marginBottom: 14, maxWidth: 560}}>
              Topics tagged with a small dot are this cohort’s weakest areas — generating here directly replenishes the booster pool.
            </p>
            <div className="col" style={{gap: 18}}>
              {topics.map(g => (
                <div key={g.group}>
                  <div className="small muted" style={{textTransform:'uppercase', letterSpacing:'.06em', fontWeight:500, marginBottom: 8}}>
                    {g.group}
                  </div>
            ...
            <div className="col" style={{gap: 12}}>
              {['Easy','Medium','Hard'].map(d => (
                <div key={d} className="row" style={{gap: 14, alignItems:'center'}}>
                  <DiffPillG level={d}/>
                  <input type="range" min="0" max="100" value={mix[d]}
                         onChange={e => setMix({...mix, [d]: Number(e.target.value)})}
                         style={{flex:1, accentColor:'var(--accent)'}}/>
                  <div className="num small" style={{width: 44, textAlign:'right', color:'var(--ink-2)'}}>{mix[d]}%</div>
                  <div className="num small" style={{width: 30, textAlign:'right', color:'var(--ink-3)'}}>·{diffCounts[d]}</div>
                </div>
              ))}
            </div>
          </SectionG>
        ...
        <div className="col" style={{gap: 16, position:'sticky', top: 80}}>
          ...
          {generated && <GeneratedList items={generated} onRedo={reset} goReview={() => go && go('review')}/>}
        </div>
      </div>
    </div>
  );
...
function SectionG({ title, hint, action, children }){
  return (
    <div>
...
function DiffPillG({ level }){
  const map = { Easy: {bg:'#EDF2ED', fg:'#406348'},
                Medium:{bg:'#F1EEE6', fg:'#6B5B37'},
                Hard:  {bg:'#F1E7E4', fg:'#7A3D35'} };
  const s = map[level] || map.Medium;
  return <span className="tag" style={{background:s.bg, borderColor:s.bg, color:s.fg, width: 70, justifyContent:'center'}}>{level}</span>;
}

function PreviewCard({ subject, breakdown, diffCounts, count, onGenerate, disabled }){
  return (
    <div className="card" style={{padding: 24}}>
      <div className="small muted" style={{textTransform:'uppercase', letterSpacing:'.08em', fontWeight:500, marginBottom: 12}}>
        Preview
      </div>
      ...
      {breakdown.length === 0 ? (
        <div style={{padding:'24px 16px', textAlign:'center', background:'var(--surface-2)', borderRadius:10, border:'1px dashed var(--border)'}}>
          <div className="muted small">Select at least one topic to preview the run.</div>
        </div>
      ) : (
        <>
          <div className="small muted" style={{marginBottom: 8}}>By topic</div>
          <div className="col" style={{gap: 6, marginBottom: 16}}>
            {breakdown.map(b => (
              <div key={b.id} className="row between" style={{padding: '8px 12px', background:'var(--surface-2)', borderRadius: 8}}>
                <span style={{fontSize:13.5, color:'var(--ink-2)'}}>{b.label}</span>
                <span className="num small" style={{color:'var(--ink)', fontWeight:500}}>× {b.n}</span>
              </div>
            ))}
          </div>

          <div className="small muted" style={{marginBottom: 8}}>By difficulty</div>
          <div className="row" style={{gap: 6, marginBottom: 18, flexWrap:'wrap'}}>
            {['Easy','Medium','Hard'].map(d => (
              <div key={d} className="row" style={{gap:6, padding:'6px 10px', background:'var(--surface-2)', borderRadius:8}}>
                <DiffPillG level={d}/>
                <span className="num small" style={{color:'var(--ink-2)', fontWeight:500}}>× {diffCounts[d]}</span>
              </div>
            ))}
          </div>
        </>
      ...
      <div className="row between small muted" style={{marginBottom: 14}}>
        <span><IClock size={13}/> Est. {Math.max(8, count * 4)}s</span>
        <span>Drafts → AI review queue</span>
      </div>
      <button className="btn btn-primary btn-lg" disabled={disabled}
              style={{width:'100%', justifyContent:'center', opacity: disabled ? .55 : 1, cursor: disabled ? 'not-allowed' : 'pointer'}}
              onClick={onGenerate}>
...
function LoadingCard({ count }){
  return (
    <div className="card" style={{padding: 28, textAlign:'center'}}>
      ...
      <div style={{display:'inline-flex', alignItems:'center', gap:10, fontFamily:'var(--serif)', fontSize: 18, marginBottom: 18}}>
      ...
      <div className="col" style={{gap: 8, textAlign:'left'}}>
        {Array.from({length: Math.min(count, 3)}).map((_,i) => (
          <div key={i} style={{padding:'12px 14px', background:'var(--surface-2)', borderRadius: 8, border:'1px solid var(--border-2)'}}>
            <div style={{height: 10, background:'var(--border)', borderRadius:4, width: '85%', marginBottom: 8}}/>
...
function GeneratedList({ items: initialItems, onRedo, goReview }){
  const [items, setItems] = useStateG(initialItems);
  const [acted, setActed] = useStateG({});  // id -> 'kept' | 'rejected'
  const [editing, setEditing] = useStateG(null); // question being edited
  const [sent, setSent] = useStateG(null);  // null | { kept, rejected }

  function act(id, k){ setActed({...acted, [id]: k}); }

  function keepAll(){
    const next = {...acted};
    items.forEach(q => { if(next[q.id] !== 'rejected') next[q.id] = 'kept'; });
    setActed(next);
  }
  function clearAll(){ setActed({}); }
  ...
  function regenerate(id){
    // Swap with a freshly drafted question on the same topic+difficulty
    setItems(items.map(q => q.id === id
      ? { ...q, id: 'AI-' + Math.floor(2100 + Math.random()*900),
          text: q.text.replace(/\.$/, '') + ' (re-drafted alternative)',
          confidence: 0.72 + Math.random()*0.25 }
      : q));
    setActed(prev => { const c = {...prev}; delete c[id]; return c; });
  }
  ...
  if(sent){
    return (
      <div className="card" style={{padding: 36, textAlign:'center'}}>
        ...
        <h3 style={{fontFamily:'var(--serif)', fontSize: 22, marginBottom: 6}}>
          Sent to review queue
        </h3>
        <p className="small muted" style={{marginBottom: 22, maxWidth: 360, margin:'0 auto 22px'}}>
          <span className="num" style={{color:'var(--ink)', fontWeight:500}}>{sent.kept}</span> drafts queued for approval.
          {sent.rejected > 0 && <> <span className="num">{sent.rejected}</span> rejected drafts were discarded.</>}
        </p>
        <div className="row" style={{gap: 10, justifyContent:'center'}}>
          <button className="btn btn-ghost" onClick={onRedo}>
            <ISpark/> Generate more
          </button>
          <button className="btn btn-primary btn-lg" onClick={() => goReview && goReview()}>
  ...
  return (
    <>
      <div className="card" style={{padding: 18, background:'var(--ok-soft)', borderColor: 'color-mix(in srgb, var(--ok) 25%, white)'}}>
        <div className="row between">
          <div className="row" style={{gap: 10}}>
            <div style={{width: 28, height: 28, borderRadius:'50%', background:'var(--ok)', color:'#fff', display:'grid', placeItems:'center'}}>
              <ICheck size={14}/>
            </div>
            <div>
              ...
              <div className="small" style={{color:'var(--ok)', opacity:.85}}>
        ...
        <div className="row" style={{gap: 8, marginTop: 14, paddingTop: 12, borderTop: '1px solid color-mix(in srgb, var(--ok) 18%, transparent)'}}>
          <button className="btn btn-ghost" onClick={keepAll} style={{background:'#fff', padding:'7px 12px', fontSize:13}}>
            <ICheck/> Keep all
          </button>
          <button className="btn btn-ghost" onClick={clearAll} style={{background:'#fff', padding:'7px 12px', fontSize:13}}>
            Clear selections
          </button>
          <div style={{flex:1}}/>
      ...
      <div className="col" style={{gap: 10, maxHeight: '60vh', overflow: 'auto', paddingRight: 4}}>
        {items.map(q => (
          <div key={q.id} className="card" style={{padding: 16, opacity: acted[q.id] === 'rejected' ? .55 : 1}}>
            <div className="row between" style={{marginBottom: 8}}>
              <div className="row" style={{gap: 6}}>
                <span className="mono small muted">{q.id}</span>
                <span className="tag tag-strong" style={{fontSize: 11}}>{q.topic}</span>
                <DiffPillG level={q.difficulty}/>
              </div>
              <span className="tag" style={{fontSize: 11, background: q.confidence >= .85 ? 'var(--ok-soft)' : 'var(--warn-soft)', color: q.confidence >= .85 ? 'var(--ok)' : 'var(--warn)', borderColor: 'transparent'}}>
                {Math.round(q.confidence*100)}% conf.
              </span>
            </div>
            <div style={{fontFamily:'var(--serif)', fontSize: 15, color: 'var(--ink)', lineHeight:1.45, marginBottom: 10}}>
              {q.text}
            </div>
            ...
            <div className="row" style={{gap: 8, justifyContent:'flex-end'}}>
              {acted[q.id] === 'kept' && <span className="tag tag-ok"><ICheck size={11}/> Queued</span>}
              {acted[q.id] === 'rejected' && <span className="tag tag-bad"><IX size={11}/> Rejected</span>}
              {acted[q.id] && (
                <button className="btn btn-quiet" onClick={() => act(q.id, null)} style={{padding:'5px 10px', fontSize:12}}>Undo</button>
              )}
              {!acted[q.id] && (
                <>
                  <button className="btn btn-ghost" title="Regenerate — replace with a fresh draft"
                          style={{padding:'7px 10px', fontSize:13}}
                          onClick={() => regenerate(q.id)}>
                    <ISpark/> Regenerate
                  </button>
                  <button className="btn btn-ghost" title="Edit this draft"
                          style={{padding:'7px 10px', fontSize:13}}
                          onClick={() => setEditing(q)}>
                    <IEdit/> Edit
                  </button>
                  <button className="btn btn-ghost"
                          style={{padding:'7px 12px', fontSize:13, color:'var(--bad)', borderColor:'color-mix(in srgb, var(--bad) 30%, var(--border))'}}
                          onClick={() => act(q.id, 'rejected')}>
                    <IX/> Reject
                  </button>
                  <button className="btn btn-primary" style={{padding:'7px 12px', fontSize:13}} onClick={() => act(q.id, 'kept')}>
                    <ICheck/> Keep
                  </button>
                </>
              )}
      ...
      <div className="card" style={{padding: 16, position:'sticky', bottom: 0, background:'var(--surface)'}}>
        <div className="row between" style={{marginBottom: 10}}>
          <span className="small muted">
            <span className="num" style={{color:'var(--ink)', fontWeight:500}}>{keptCount}</span> kept ·{' '}
            <span className="num">{rejectedCount}</span> rejected ·{' '}
            <span className="num">{items.length - keptCount - rejectedCount}</span> pending
          </span>
        </div>
        <button className="btn btn-primary btn-lg"
                style={{width:'100%', justifyContent:'center', opacity: keptCount===0 ? .55 : 1, cursor: keptCount===0 ? 'not-allowed' : 'pointer'}}
...
/* ---------- Edit draft modal ---------- */
function EditDraftModal({ question, onSave, onClose }){
  const [text, setText]               = useStateG(question.text);
  const [topic, setTopic]             = useStateG(question.topic);
  const [difficulty, setDifficulty]   = useStateG(question.difficulty);
  const [choices, setChoices]         = useStateG(question.choices);
  const [correctKey, setCorrectKey]   = useStateG(question.correctKey);

  function setChoice(idx, t){
    setChoices(choices.map((c,i) => i === idx ? {...c, t} : c));
  }
  ...
  return (
    <div onClick={onClose} style={{position:'fixed', inset:0, background:'rgba(20,20,20,0.4)', display:'grid', placeItems:'center', zIndex:50, padding:24}}>
      <div onClick={stop} className="card" style={{width:'min(680px, 100%)', maxHeight:'90vh', overflow:'auto', padding:0, boxShadow:'var(--shadow-lg)'}}>
          ...
          <div className="col" style={{gap:8}}>
            ...
            {choices.map((c, i) => (
              <div key={c.k} className="row" style={{gap:10, alignItems:'center'}}>
                <button onClick={() => setCorrectKey(c.k)}
                ...
                <input type="text" value={c.t} onChange={e => setChoice(i, e.target.value)} style={{flex:1}}/>
                {correctKey===c.k && <span className="tag tag-ok" style={{fontSize:11}}>Correct</span>}
              </div>
            ))}
          </div>
        </div>
...
/* ---------- Mock generator ---------- */
function mockGenerate(breakdown, diffCounts, subject){
  const samples = {
    'Transitions': [
    ...
    'Quadratics': [
      "If x² − 5x − 14 = 0, what is the sum of the solutions?",
    ],
    'Word in context': [
      "As used in the passage, “______” most nearly means ‘intentionally restrained.’",
    ],
    'Central idea': [
  ...
  const choicesPool = [
    [{k:"A",t:"However,"},{k:"B",t:"As a result,"},{k:"C",t:"For example,"},{k:"D",t:"Likewise,"}],
    [{k:"A",t:"Moreover,"},{k:"B",t:"Nevertheless,"},{k:"C",t:"Therefore,"},{k:"D",t:"Similarly,"}],
    [{k:"A",t:"4"},{k:"B",t:"−4"},{k:"C",t:"5"},{k:"D",t:"−5"}],
  ];

  // Build the difficulty pool to draw from
  const diffPool = [
  ...
  for(const b of breakdown){
    for(let i = 0; i < b.n; i++){
      const sampList = samples[b.label] || ["A newly drafted SAT-style question for " + b.label + " will appear here."];
